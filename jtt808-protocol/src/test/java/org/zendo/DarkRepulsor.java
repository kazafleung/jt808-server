package org.zendo;

import io.netty.buffer.ByteBuf;
import io.netty.buffer.ByteBufUtil;
import io.netty.buffer.Unpooled;
import org.zendo.protocol.basics.JTMessage;
import org.zendo.protocol.codec.JTMessageDecoder;
import org.zendo.protocol.codec.JTMessageEncoder;

/**
 * 压力测试
 * @author yezhihao
 * https://gitee.com/yezhihao/jt808-server
 */
public class DarkRepulsor {

    private static JTMessageDecoder decoder = new JTMessageDecoder("org.zendo.protocol");
    private static JTMessageEncoder encoder = new JTMessageEncoder("org.zendo.protocol");

    //560
    public static void main(String[] args) {
        String hex = "";
        ByteBuf buf = Unpooled.wrappedBuffer(ByteBufUtil.decodeHexDump(hex));

        while (true) {
            long s = System.currentTimeMillis();

            for (int i = 0; i < 100000; i++) {
                JTMessage message = decoder.decode(buf);
                message.setSerialNo(message.getSerialNo() + 1);

                buf.release();
                buf = encoder.encode(message);
            }
            System.out.println(System.currentTimeMillis() - s);
        }
    }
}